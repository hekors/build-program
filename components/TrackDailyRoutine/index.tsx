...
} from '@/types/track-daily-routine-type';
...
const TrackDailyRoutine: React.FunctionComponent<TrackDailyRoutinePropType> = (
  { trackSlugID },
  props: any,
) => {
  const [trackDailyRoutineData, setTrackDailyRoutineData] =
    useState<TrackDailyRoutineType>(undefined);
  ...
  return (
    <SectionContainer
      className="track-daily-routine-section flex flex-row items-start justify-around"
      {...props}>
      <Container className="track-daily-routine-details-section flex flex-col items-start justify-start gap-6">
        ...
        {trackDailyRoutineData?.trackDailyRoutineTimeline?.workingDays
          ?.startingWorkDay ? (
          <Container className="working-days-details-wrapper mt-6">
            <Headline className="font-semibold text-gray-900 text-lg">
              {'Working Days'}
            </Headline>
            <WrappedContainer>
              <Paragraph>
      ...
      <Container className="track-daily-routine-timeline-flow-wrapper">
        <Container className="track-daily-routine-timeline-flow-container flex flex-col items-start justify-start">
          {/* @ts-ignore */}
          {trackDailyRoutineData?.trackDailyRoutineTimeline?.workingDays?.flow?.map(
            (routineTask, routineTaskIndex: number) => (
              <Container
                className="relative p-6 border-l-4 border-gray-900"
                key={routineTaskIndex}>
                <WrappedContainer
                  className="routine-task-slot_time-icon-layer-wrapper absolute flex flex-row 
                                items-center justify-end gap-6">
                  <Paragraph
                    className="text-sm font-normal text-gray-500 absolute
                                    top-1 -left-[120px]">
                    {routineTask?.timestamp}
                  </Paragraph>
                  <WrappedContainer
                    className="w-6 h-6 border-4 border-gray-100 bg-gray-900 absolute rounded-full 
                                    top-0 -left-[38px]
                                "
                  />
                </WrappedContainer>
                <Container className="routine-task-slot-content-container w-[320px] flex flex-col items-start justify-start gap-2">
                  <Headline className="text-xl font-semibold">
                    {routineTask?.slotTitle}
                  </Headline>
